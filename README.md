...
| POST | `/v1/servicing-session-operating-session/initiate` | Initiate |
| GET | `/v1/servicing-session-operating-session` | Retrieve (list) |
| GET | `/v1/servicing-session-operating-session/{crId}/retrieve` | Retrieve |
| PUT | `/v1/servicing-session-operating-session/{crId}/update` | Update |
| PUT | `/v1/servicing-session-operating-session/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/servicing-session-operating-session/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-customer-workbench:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-customer-workbench ./helm -n bian-sales-service
```

Exposed through the platform Gateway at path prefix `/sd-customer-workbench` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
